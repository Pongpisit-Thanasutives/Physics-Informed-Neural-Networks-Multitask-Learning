...
class FBSNN(ABC):
    ...
    def loss_function(self, t, W, Xi):
        ...
        for n in range(0, self.N):
            ...
            X1_t2 = X0 + self.mu_tf(t0, X0_t2, Y0_t2, Z0_t2) * (t1 - t0) + torch.squeeze(
                torch.matmul(self.aux_sigma_tf(t0, X0_t2, Y0_t2), (W1 - W0).unsqueeze(-1)), dim=-1)
            ...
            Y1_tilde_t2 = Y0_t2 + self.phi_tf(t0, X0_t2, Y0_t2, Z0_t2) * (t1 - t0) + torch.sum(
                Z0_t2 * torch.squeeze(torch.matmul(self.aux_sigma_tf(t0, X0_t2, Y0_t2), (W1 - W0).unsqueeze(-1))), dim=1,
                keepdim=True)
        ...
        loss2 += torch.sum(torch.pow(Y1_t2 - self.g_tf(X1_t2), 2))
        loss2 += torch.sum(torch.pow(Z1_t2 - self.Dg_tf(X1_t2), 2))

        X = torch.stack(X_list, dim=1)
        Y = torch.stack(Y_list, dim=1)
        
        total_loss += torch.sum(torch.exp(-self.log_vars[0])*loss1 + self.log_vars[0], -1)
        total_loss += torch.sum(torch.exp(-self.log_vars[1])*loss2 + self.log_vars[1], -1)
    ...
    @abstractmethod
    def aux_sigma_tf(self, t, X, Y):  # M x 1, M x D, M x 1
        M = self.M
        D = self.D
        return torch.diag_embed(torch.ones([M, D])).to(self.device)  # M x D x D
    ###########################################################################